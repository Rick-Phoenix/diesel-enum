...
use syn::{
  parse::Parse, parse_macro_input, Error, Expr, Ident, ItemEnum, Lit, MetaNameValue, Path, Token,
};
  ...
  fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
    ...
    for arg in punctuated_args {
      ...
      if ident == "case" {
        ...
        conn = Some(extract_path(value)?);
      } else {
        return Err(spanned_error!(
          ident,
          format!(
            "Unknown attribute `{}`, expected one of: `table`, `column`, `conn`",
            ident
          )
        ));
      }
    ...
    let conn = conn.ok_or_else(|| error!(input.span(), "Missing required `conn` attribute"))?;
...
pub fn diesel_sqlite_enum(attrs: TokenStream, input: TokenStream) -> TokenStream {
  ...
  let connection_func = attributes.conn;
  let table_name = attributes
    .table
    .unwrap_or_else(|| enum_name_str.to_case(Case::Snake));
  let table_name_ident = format_ident!("{table_name}");
  let column_name = attributes.column.unwrap_or_else(|| "name".to_string());
  let column_name_ident = format_ident!("{column_name}");

  let display_impl_body = traverse_enum(&variant_idents, |variant| {
    quote! {
      Self::#variant => write!(f, stringify!(#variant))
    }
  });

  let try_from_str_body = traverse_enum(&variant_idents, |variant| {
    quote! {
  ...
  let output = quote! {
    ...
    impl diesel::deserialize::FromSql<diesel::sql_types::Text, diesel::sqlite::Sqlite> for #enum_name {
      fn from_sql(bytes: diesel::sqlite::SqliteValue) -> diesel::deserialize::Result<Self> {
        let t = <String as diesel::deserialize::FromSql<diesel::sql_types::Text, diesel::sqlite::Sqlite>>::from_sql(bytes)?;
